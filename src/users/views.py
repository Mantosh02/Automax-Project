  ...
  if request.method=='POST':
    ...
    if login_form.is_valid():
      ...
      if user is not None:
        ...
        messages.error(request,f'An error occured trying to log in as {username}.')
    else:
...
class RegisterView(View):
  ...
  def post(self,request):
    ...
    if register_form.is_valid():
      ...
      login(request,user)
      messages.success(request,f'you are now resgistered in as {user.username}.')
      return redirect('home')
    else:
...
class ProfileView(View):
  ...
  def post(self,request):
    ...
    if user_form.is_valid() and profile_form.is_valid() and location_form.is_valid():
      ...
      messages.error(request,f'An error occured trying to update your profile.')
    return render(request,'views/profile.html' ,{'user_form':user_form , 'profile_form':profile_form,'location_form':location_form,'user_listings':user_listings,'user_liked_listings':user_liked_listings})
