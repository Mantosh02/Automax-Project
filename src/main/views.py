...
#     'eryadav2002@gmail.com',  # from email
#     ['mrmantoshkumar2002@gmail.com'],  # recipient email
#     fail_silently=False,
# )
...
def list_view(request):
  if request.method=='POST':
    try:
      ...
      print(e)
      messages.error(request,'An error occured while posting the listing. Please try again later.')
  ...
  try:
    ...
    if request.method=='POST':
      ...
      if listing_form.is_valid and location_form.is_valid():
        listing_form.save()
        location_form.save()
        messages.info(request,f'{listing.model} Listing Edited Successfully!')
        return redirect('home')
      else:
        messages.error(request,f'An error occured while editing the listing. Please try again later.')
        return reload()
    else:
...
@login_required
def inquir_listing_using_email(request,id):
  listing=get_object_or_404(Listing,id=id)
  try:
    emailSubject=f"{request.user.username} is entrested in this {listing.model}"
    emailMessage=f"Hi {listing.seller.user.username} ,{request.user.username} is intrested in your {listing.model} listing on AutoMax."
